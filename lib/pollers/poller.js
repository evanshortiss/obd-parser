 ...
 * @return {Number}
 */
function getPollDelay (max, lastPollTs) {
  return (max - lastPollTs);
}
...
ECUPoller.prototype._onEcuData = function (data) {
  if (data.byteString.indexOf(this.pidInfo.opts.pid) === 2) {
...
ECUPoller.prototype._queuePoll = function () {
  if (this.polling) {
    ...
    var delay = getPollDelay(
      this.pollInterval,
      (this.pollTimer) ? this.pollTimer._idleStart : 0
    );
